...
def import_stored_files():
    # Load in the stored Epicurious database, OHE/CountVectorizer object to transform,
    # the input, and the OHE/CountVectorizer word matrix from joblib and created by
    # prepare_database.py
    with open("joblib/ohe_recipe_dataframe.joblib", "rb") as fo:
        prepped = joblib.load("joblib/ohe_recipe_dataframe.joblib")

    with open("joblib/recipe_ohe.joblib", "rb") as fo:
        ingred_ohe = joblib.load("joblib/recipe_ohe.joblib")

    with open("joblib/recipe_word_matrix_ohe.joblib", "rb") as fo:
        ingred_word_matrix = joblib.load("joblib/recipe_word_matrix_ohe.joblib")

    return prepped, ingred_ohe, ingred_word_matrix
...
def transform_ohe(ingred_ohe, recipe):
    ...
    transformed_recipe = pd.DataFrame(
        response.toarray(), columns=ingred_ohe.get_feature_names(), index=recipe.index
    )
...
def filter_out_cuisine(ingred_word_matrix, X_df, cuisine_name, ohe):
    ...
    # name, and the ingredient OHE/CountVectorizer object (from joblib) and returns
...
def find_closest_recipes(filtered_ingred_word_matrix, recipe_ohe, X_df):
    # This function takes in the filtered ingredient word matrix from function
    # filter_out_cuisine, the ohe recipe from function transform_ohe, and
...
def find_similar_dishes(dish_name, cuisine_name):
    prepped, ingred_ohe, ingred_word_matrix = import_stored_files()
    ...
    if resp.status_code == 200:
        ...
        query_ohe = transform_ohe(ingred_ohe=ingred_ohe, recipe=query_df)
        ...
        query_similar, ingreds_used, rec_weights = find_closest_recipes(
            filtered_ingred_word_matrix=query_matrix, recipe_ohe=query_ohe, X_df=prepped
        )
